...
int main()
{
	...
	*/
	...
	camera cam(w_Width, w_Height, glm::vec3(0.0f, 0.0f, 8.0f));
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (unsigned int i = 0; i < 1; i++) {
			...
			view = glm::translate(view, Pyramid[i]);
			proj = glm::perspective(glm::radians(45.0f), (float)(w_Width / w_Height), 0.1f, 100.0f);


			int modelLoc = glGetUniformLocation(shaderProgram.ID, "model");
			glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));  //Gl_false = no matrix transpose ..for specifying row major 
